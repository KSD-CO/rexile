...
fn multi_pattern_benchmark(c: &mut Criterion) {
    ...
    let patterns = vec![
        ...
        ("8_alternatives", "import|export|function|return|class|const|let|var"),
    ];
...
fn find_operations_benchmark(c: &mut Criterion) {
    ...
    group.bench_function("find_all", |b| {
        b.iter(|| {
            black_box(pattern.find_all(black_box(text)))
        });
    });
...
fn cached_api_benchmark(c: &mut Criterion) {
    ...
    group.bench_function("cached_is_match", |b| {
        b.iter(|| {
            black_box(rexile::is_match("test", black_box(text)).unwrap())
        });
    });
    
    group.bench_function("cached_find", |b| {
        b.iter(|| {
            black_box(rexile::find("test", black_box(text)).unwrap())
        });
    });
...
fn text_size_scaling_benchmark(c: &mut Criterion) {
    ...
    group.bench_function("small_100_bytes", |b| {
        b.iter(|| {
            black_box(pattern.is_match(black_box(&small_text)))
        });
    });
    
    group.bench_function("medium_1kb", |b| {
        b.iter(|| {
            black_box(pattern.is_match(black_box(&medium_text)))
        });
    });
    
    group.bench_function("large_10kb", |b| {
        b.iter(|| {
            black_box(pattern.is_match(black_box(&large_text)))
        });
    });
