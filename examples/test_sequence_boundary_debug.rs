    ...
    match Pattern::new(pattern_str) {
        Ok(pattern) => {
            ...
            if did_match == should_match {
                println!("✓ PASS ({})", if did_match { "matched" } else { "no match" });
            } else {
                ...
                if let Some((start, end)) = pattern.find(text) {
                    println!("     Found at [{}, {}) = {:?}", start, end, &text[start..end]);
                }
            }
        }
        Err(e) => {
...
fn main() {
    ...
    test_match(r"\bhello", "xhello", false, r"\bhello in 'xhello' (no boundary)");
    test_match(r"\bhello", " hello", true, r"\bhello in ' hello'");

    test_match(r"hello\b", "hello world", true, r"hello\b in 'hello world'");
    test_match(r"hello\b", "hellox", false, r"hello\b in 'hellox' (no boundary)");

    test_match(r"\bworld", "hello world", true, r"\bworld in 'hello world'");
    test_match(r"\bworld", "helloworld", false, r"\bworld in 'helloworld' (no boundary)");

    test_match(r"\bhello\b", "hello", true, r"\bhello\b in 'hello'");
    test_match(r"\bhello\b", "hello world", true, r"\bhello\b in 'hello world'");
    test_match(r"\bhello\b", "xhellox", false, r"\bhello\b in 'xhellox' (no boundaries)");
