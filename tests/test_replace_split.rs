...
fn test_replace_simple() {
    ...
    // Replace first word
    assert_eq!(pattern.replace("hello world", "goodbye"), "goodbye world");

    // No match
    assert_eq!(pattern.replace("123 456", "xxx"), "xxx 456");
}
...
fn test_replace_with_captures() {
    ...
    assert_eq!(pattern.replace("a=1 b=2", "$1:[$2]"), "a:[1] b=2");

    // Multiple groups
    assert_eq!(pattern.replace("foo=42 bar=99", "$2=$1"), "42=foo bar=99");
}
...
fn test_replace_no_match() {
    ...
    assert_eq!(pattern.replace("no numbers here", "XXX"), "no numbers here");
}
...
fn test_replace_all_simple() {
    ...
    // Replace all digits
    assert_eq!(pattern.replace_all("a1b2c3", "X"), "aXbXcX");
}
...
fn test_replace_literal_dollar() {
    ...
    assert_eq!(pattern.replace("hello", "$price"), "$price");
}
...
fn test_replace_all_no_match() {
    ...
    // No match, return original
    assert_eq!(pattern.replace_all("no numbers", "X"), "no numbers");
}
